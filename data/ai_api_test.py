...
# %%
def get_query(model, query, vectordb_choice, collection_name, text_key="text", prompt="LG_PROMPT", search_name="Similarity", k_value=4, **kwargs) -> list[str]:
    ...
    retrieval_qa(llm, query, prompt)

    #docsearch = retriever.get_relevant_documents(query=query)
...
# %%
query = "What is this document about and what is its purpose?"
collection_name = "Tae_Kim"
...
def retrieval_qa(llm: ChatOpenAI, query: str, prompt: str, retriever, chain_type: str ="stuff", return_source_documents: bool =False)-> dict[str]:
    ...
    if chain_type in ["map_reduce", "refine",]:
        ...
        qa_chain = load_qa_chain(llm, chain_type=chain_type,  return_intermediate_steps=True)
    else:
        prompt_template = prompt_selector(prompt)
        qa_chain = load_qa_chain(llm, chain_type=chain_type, prompt=prompt_template, memory=memory)
    qa = RetrievalQA(combine_documents_chain=qa_chain, retriever=retriever, verbose=True,  return_intermediate_steps=True, return_source_documents=return_source_documents)
    return qa(query)

results = retrieval_qa(llm, query, prompt, retriever, chain_type="map_rerank", return_only_outputs=True, return_source_documents=True)
